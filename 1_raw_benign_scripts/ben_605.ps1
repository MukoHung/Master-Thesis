...
##########################################################################
# Habilitar monitorizaci�n
##########################################################################

<#
    Crea el EventSource Challenger4SysAdmins en el Registro de Windows donde se registrar�
    un evento por cada comando ejecutado en los int�rprete de comandos.
...
<#
    Habilita la monitorizaci�n de CMD (S�mbolo del sistema):
...
function Enable-CmdAudit() {
    ...
    # Configura la ejecuci�n autom�tica del script al iniciar CMD para monitorizarlo
    Write-Host "- Configurando valor AutoRun en el registro para iniciar script de monitorizaci�n al abrir CMD"
    New-ItemProperty -Path $CmdAutorunPath -Name Autorun -PropertyType ExpandString -Value $CmdAuditScriptPath | Out-Null
}

<#
    Habilita la monitorizaci�n de PowerShell
#>
function Enable-PSAudit() {  
    # Crea el perfil PowerShell global (para todos los usuarios) que habilita su monitorizaci�n sobrescribiendo la funci�n "prompt()"
...
<#
    Comprueba si est� habilitada la monitorizaci�n de CMD (S�mbolo del sistema)
#>
function Test-CmdAudit() {
    ...
    # y adem�s ese valor hace referencia al script de monitorizaci�n.
...
<#
    Comprueba si est� habilitada la monitorizaci�n de PowerShell
#>
...
<#
    Deshabilita la monitorizaci�n de CMD (S�mbolo del sistema)
#>
function Disable-CmdAudit() {
    ...
    Write-Host "- Eliminando valor AutoRun en el registro para iniciar script de monitorizaci�n al abrir CMD"
    Remove-ItemProperty -Path $CmdAutorunPath -Name Autorun | Out-Null
}

<#
    Deshabilita la monitorizaci�n de PowerShell
#>
...
If ($Test) {
    ...
    Write-Host -NoNewline "- Comprobando si est� habilitada la monitorizaci�n de CMD "
    ...
    Write-Host -NoNewline "- Comprobando si est� habilitada la monitorizaci�n de PowerShell "
    if (-not (Test-PSAudit)) {
        Write-Host "[NO]"
        $ReturnValue = 1
    } else {
