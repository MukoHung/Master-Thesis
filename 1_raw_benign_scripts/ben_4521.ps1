...
if (-not(Test-Path -Path "C:\Windows\Temp\$($PostScript)" -PathType Leaf)) {
    Copy-Item $PostScriptPath -Destination "C:\Windows\Temp\$($PostScript)"
}
    ...
    switch ($global:RestartRequired) {
        0 {
            ...
            if (($global:MoreUpdates -eq 1) -and ($script:Cycles -le $global:MaxCycles)) {
                ...
                LogWrite "Running $($PostScript)..."
                $result = & "C:\Windows\Temp\$($PostScript)"
                LogWrite "Completed run of $($PostScript) with $($result)"
            } else {
