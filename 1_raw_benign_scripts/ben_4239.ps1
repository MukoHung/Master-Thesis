...
.DESCRIPTION
   ...
   ##RemoveAD-Tool from Name of the script##
.EXAMPLE
  PasswordChangeNotification.ps1 -smtpServer mail.pittsh.com.au -expireInDays 21 -from "IT Support <support@domain.com>" -Logging -LogPath "c:\logFiles" -testing -testRecipient support@domain.com
.EXAMPLE
...
$users = get-aduser -filter {(Enabled -eq $true) -and (PasswordNeverExpires -eq $false)} -properties Name, DisplayName, PasswordNeverExpires, PasswordExpired, PasswordLastSet, EmailAddress | where-object { $_.passwordexpired -eq $false }
# Count Users
...
foreach ($user in $notifyUsers)
{
    ...
    <ul><li>If working remotely, please log into https://login.microsoftonline.com </li></ul><br>
    <br>
    <br>
    <p> Remember to update the password on your Mobile Devices as well!<br>
    </P>
    <br>
    <p>
    <p>Kind Regards, <br> 
    ...
    IT Support
    <a href=""mailto:Helpdesk@pittsh.com.au""?Subject=Password Expiry Assistance"">Helpdesk@pittsh.com.au</a>
    </font>"
...
$notifiedUsers | Select-Object UserName,Name,EmailAddress,PasswordSet,DaysToExpire,ExpiresOn | sort DaystoExpire | FT -autoSize
