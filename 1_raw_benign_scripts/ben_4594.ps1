# (C) 2020-2022 Gordon Engelke <reject@email.de>
#
...
function az-set($env) {
    If ($env -eq 'dev' -Or $env -eq 'qa' -Or $env -eq 'prod') {
        ...
        if (!$context) {
            ...
            Connect-AzAccount -Subscription CI-OSE3-BoschDevCloud_Shared-$env | Out-null
        } else {
            Write-Host "Already connected to Azure account." -ForegroundColor Green
        }

        if ($context.Subscription.Name -ne "CI-OSE3-BoschDevCloud_Shared-$env") {
            Write-Host "Connecting to subscription CI-OSE3-BoschDevCloud_Shared-$env..." -ForegroundColor Blue
            Select-AzSubscription -SubscriptionName "CI-OSE3-BoschDevCloud_Shared-$env" | Out-null
            Get-AzSubscription -SubscriptionName "CI-OSE3-BoschDevCloud_Shared-$env" | Set-AzContext | Out-null
        } else {
            Write-Host "Already connected to subscription CI-OSE3-BoschDevCloud_Shared-$env." -ForegroundColor Green
        }
        ...
        if ($(az account show --query name -o tsv) -ne "CI-OSE3-BoschDevCloud_Shared-$env") {
            Write-Host "Setting active subscription to CI-OSE3-BoschDevCloud_Shared-$env..." -ForegroundColor Blue
            az account set -s CI-OSE3-BoschDevCloud_Shared-$env; 
        }
        else {
            Write-Host "Already using subscription CI-OSE3-BoschDevCloud_Shared-$env." -ForegroundColor Green
        }
        Write-Host "Setting AKS credentials..." -ForegroundColor Blue
        az aks get-credentials --resource-group $env-shared-02-aks-rg --name $env-shared-02-aks --subscription CI-OSE3-BoschDevCloud_Shared-$env 
    } else {
        Write-Host("Invalid environment given: '$env'. Must be 'dev', 'qa' or 'prod'.") -ForegroundColor Red
    }
    $env:bdcenv = $env
}
