...
if ($($args.Count) -ne 1) {
    ...
    if ($($args[0]) -eq "-f") {
        ...
        foreach ($f in Get-ChildItem $SOURCE_PCAP_DIR) {
            ...
            # For Linux
            mono ./0_Tool/SplitCap_2-1/SplitCap.exe -p 1018 -b 1018 -r $x -s flow -o ./2_Session/AllLayers/$($f.BaseName)-ALL
            Get-ChildItem ./2_Session/AllLayers/$($f.BaseName)-ALL | ?{$_.Length -eq 0} | Remove-Item
            #mono ./0_Tool/SplitCap_2-1/SplitCap.exe -p 1018 -b 1018 -r $x -s flow -o ./2_Session/L7/$($f.BaseName)-L7
            #Get-ChildItem ./2_Session/L7/$($f.BaseName)-L7 | ?{$_.Length -eq 0} | Remove-Item
        }
