...
[string]$ipMonNotes = $config.ipMonNotes
...
"@
    ...
    if ($ipmonenabled) {
        $htmlBody += @"
<button class="tablinks" onclick="openTab(event,'ipmon')">IP Monitor</button>
"@
    }
...
$SkuNames = @{
    ...
    "Win10_VDA_E3"                                = "Windows 10 Enterpise E3"
...
$allLicences = @()
$allLicences = Get-GraphResults $uriLicences $authGraphHeader $proxyserver $proxyHost
...
if (Test-Path $pathdata) {
    ...
    if ($currentdata.count -lt 1) {
        ...
        Write-ELog -useEventLog $useEventLog -LogName $evtLogname -Source $evtSource -Message "$($rptProfile) : $evtMessage" -EventId 705 -EntryType Error
        Write-Log $evtMessage
    }
}
else {
    ...
    $emailAlert += $evtMessage + "`r`n"
    Write-ELog -useEventLog $useEventLog -LogName $evtLogname -Source $evtSource -Message "$($rptProfile) : $evtMessage" -EventId 704 -EntryType Warning
    Write-Log $evtMessage
}
...
Write-ELog -useEventLog $useEventLog -LogName $evtLogname -Source $evtSource -Message "$($rptProfile) : $evtMessage" -EventId 701 -EntryType Information
Write-Log $evtMessage
...
if (($version.latest -gt $lastVersion) -or ($null -like $currentData) -or $fileMissing) {
    ...
    Write-ELog -useEventLog $useEventLog -LogName $evtLogname -Source $evtSource -Message "$($rptProfile) : $evtMessage" -EventId 702 -EntryType Information
    Write-Log $evtMessage
    ...
    #Adds
    $flatAddChanges = $changes | Where-Object { $_.add -ne $null }
    $results = @()
    ForEach ($item in $flatAddChanges) {
        if ($null -ne $item.add.effectiveDate) { $dtmED = [datetime]::parseexact($($item.add.effectiveDate), 'yyyyMMdd', $null).tostring('dd MMM yyyy') } else { $dtmED = $null }
        $addCustomObjects = [PSCustomObject]@{
            id            = $item.id;
            action        = "Add";
            effectiveDate = $dtmED;
            ips           = $item.add.ips -join ", ";
            urls          = $item.add.urls -join ", ";
        }
        $results += $addCustomObjects
    }
    $flatAddChanges = $null
    $flatAddChanges = $results
    ...
    Write-ELog -useEventLog $useEventLog -LogName $evtLogname -Source $evtSource -Message "$($rptProfile) : $evtMessage" -EventId 703 -EntryType Information
    Write-Log $evtMessage
...
if ($cnameenabled) {
    ...
    foreach ($dns in $cnameresolvers) {
        try { $dnsResults = Import-Csv "$($pathWorking)\$cnameFilename-$($DNS)-$($rptProfile).csv" }
            ...
            Write-ELog -useEventLog $useEventLog -LogName $evtLogname -Source $evtSource -Message "$($rptProfile) : $evtMessage" -EventId 601 -EntryType Error
            Write-Log $evtMessage
        }
...
#CNAME checks where possible
$divSix = "<!- Start of section six ->"
$rptSectionSixOne = ""
if ($ipMonenabled) {
    [array]$ipMonResults = $null
    [array]$ipAddresses = $null

    #Import the DNS results being output by the monitor script
    foreach ($dns in $ipMonresolvers) {
        $dnsResults = $null
        try { $dnsResults = Import-Csv "$($pathWorking)\$($ipMonFilename)-$($DNS)-$($rptProfile).csv" }
        catch {
            $evtMessage = "[IPMonitor] Cannot import IP results file $($pathWorking)\$($ipMonFilename)-$($DNS)-$($rptProfile).csv`r`n Check file has been created by monitoring script."
            Write-ELog -useEventLog $useEventLog -LogName $evtLogname -Source $evtSource -Message "$($rptProfile) : $evtMessage" -EventId 601 -EntryType Error
            Write-Log $evtMessage
        }
        $ipMonResults += $dnsResults
    }

    $ipAddresses = $ipMonResults

    #Build Div6
    $rptSectionSixOne = "<div class='section'><div class='header'>Information</div>`n"
    $rptSectionSixOne += "<div class='content'>`n"
    $rptSectionSixOne += "$($ipMonNotes)"
    $rptSectionSixOne += "</div></div>`n"
    $divSix += $rptSectionSixOne

    $rptSectionSixTwo = "<div class='section'><div class='header'>IP Monitor</div>`r`n"
    $rptSectionSixTwo += "<div class='content'>`r`n"
    #Get ipAddresses
    #For each unique monitor
    foreach ($url in $ipMonURLs) {
        $rptIPAddressInfo += "<div class='section'>`r`n"
        $rptIPAddressInfo += "<div class='header'>$($url)</div>`r`n"
        $rptIPAddressInfo += "<div class='tableCname-header'>`r`n"
        $rptIPAddressInfo += "<div class='tableCname-header-hn'>Name</div>`r`n"
        $rptIPAddressInfo += "<div class='tableCname-header-dom'>IP</div>`r`n"
        #Now build headers for each of the resolving servers
        foreach ($dns in $ipMonresolvers) {
            $dnsServerDesc = $ipMonresolverdesc[[array]::indexof($ipMonresolvers, $DNS)]
            $rptIPAddressInfo += "<div class='tableCname-header-dtf'>$($dnsServerDesc)<br/>$($dns)<br/>First Seen</div>`r`n"
            $rptIPAddressInfo += "<div class='tableCname-header-dtl'><br/><br/>Last Seen</div>"
        }
        $rptIPAddressInfo += "</div>`r`n"
        $ipMonlist = $ipAddresses | Where-Object { $_.monitor -like $url } | Select-Object -Unique name, ipAddress | Sort-Object { $_.ipAddress -as [Version] }, name
        foreach ($ipResult in  $ipMonlist) {
            $rptIPAddressInfo += "<div class='tableCname-row'>`r`n"
            $rptIPAddressInfo += "<div class='tableCname-cell-hn'>$($ipResult.name)</div>"
            $rptIPAddressInfo += "<div class='tableCname-cell-dom'>$($ipResult.ipAddress)</div>`r`n"
            foreach ($dns in $ipMonresolvers) {
                $spotted = $ipAddresses | Where-Object { $_.resolver -like $dns -and $_.monitor -like $url -and $_.ipaddress -like $ipResult.ipaddress -and $_.name -like $ipResult.name }
                $addedDate = ""
                $lastDate = ""
                if ($spotted.addedDate) {
                    $fontcolour = "<p>" 
                ...
                $rptIPAddressInfo += "<div class='tableCname-cell-dtf'>$($addedDate)</div>`r`n"
                if ($spotted.lastdate) {
                    $fontcolour = "<p class='ok'>" 
                    if ((Get-Date $spotted.lastdate) -lt ((Get-Date).addhours(-12))) { $fontcolour = "<p class='error'>" }
                    elseif ((Get-Date $spotted.lastdate) -lt ((Get-Date).addhours(-4))) { $fontcolour = "<p class='warning'>" }
                    else { $fontcolour = "<p class='ok'>" }
                    $lastDate = "$($fontcolour)$(Get-Date $spotted.lastDate -Format 'dd-MMM-yy HH:mm')</p>"
                ...
                $rptIPAddressInfo += "<div class='tableCname-cell-dtl'>$($lastDate)</div>`r`n"
            }
            $rptIPAddressInfo += "</div>`r`n"
        }
        $rptIPAddressInfo += "</div>`r`n"
        #$rptIPAddressInfo += "<div><br/></div>`r`n"
    }
    $rptIPAddressInfo += "</div>`r`n"
    #get name host and added date


    $rptSectionSixTwo += $rptIPAddressInfo
...
BuildHTML $rptTitle $divOne $divTwo $divThree $divFour $divFive $divSix $divLast $swScript.Elapsed $rptHTMLName
...
if ($emailAlert -and $emaiLEnabled) {
    ...
    $emailAlert += $evtMessage + "`r`n"
    Write-ELog -useEventLog $useEventLog -LogName $evtLogname -Source $evtSource -Message "$($rptProfile) : $evtMessage" -EventId 102 -EntryType Warning
    Write-Log $evtMessage
