...
$ScriptVersion = "1.92.02 (January 20, 2022)"
...
Function CM_HPIAPackage {
    ...
    if ( Test_CMConnection ) {
        ...
        #if ( $DebugMode ) { CMTraceLog -Message "... setting HPIA Package to Version: $($Script:v_OSVER), path: $($pRepoPath)" -Type $TypeDebug }
...
Function CM_RepoUpdate {
    ...
    #--------------------------------------------------------------------------------
    if ( $DebugMode ) { CMTraceLog -Message "... setting CM Package to Version: $($Script:v_OSVER), path: $($pRepoPath)" -Type $TypeDebug }
                            ...
                            ForEach ( $iEntry in $lAddOnsList ) {
                                ForEach ( $iSoftpaq in $lSoftpaqList ) {
                                    if ( [string]$iSoftpaq.Name -match $iEntry ) {
                                        ...
                                        if ( $DebugMode ) { CMTraceLog -Message  "`tGet-SoftpaqMetadataFile done: $($ret)"  -Type $TypeWarn }
                                    } # if ( [string]$iSoftpaq.Name -match $iEntry )
                                } # ForEach ( $iSoftpaq in $lSoftpaqList )
                            } # ForEach ( $lEntry in $lAddOnsList )
...
[CmdletBinding()]
    ...
    foreach ( $cat in $Script:v_FilterCategories ) {
        if ( $pGrid.Rows[$pRow].Cells[$cat].Value ) {
            CMTraceLog -Message  "... adding filter: -Platform $pModelID -os $OS -osver $Script:v_OSVER -category $($cat) -characteristic ssm" -Type $TypeNoNewline
            $lRes = (Add-RepositoryFilter -platform $pModelID -os $OS -osver $Script:v_OSVER -category $cat -characteristic ssm 6>&1)
...
#=====================================================================================
<#
    Function Empty_Rooted_Repos

    This function creates empty repositories that include reference files
    ... makes sure each entry in the UI grid list has a repository initialized
    ... regardless of filters applied

    the idea is to allow a repository be created with all HPIA required information 
    ... and allow an organization to fill in the Softpaqs needed for the platform
#>
Function Empty_Rooted_Repos {
param( $pGrid, $pPath )

    CMTraceLog -Message  "... [Empty_Rooted_Repos] - <$pPath> - please wait !!!" -Type $TypeNorm

    $lCurrentSetLoc = Get-Location

    #----------------------------------------------------------------------------
    # for each model/platform in the grid,
    # ... starting at the rooted path,
    # ... create an HPIA repository that also contains the current reference file
    # ... with NO Softpaqs
    #----------------------------------------------------------------------------
    for ( $i = 0; $i -lt $pGrid.RowCount; $i++ ) {

        $lSysId = $pGrid[1,$i].Value                        # column 1 has the Model/Prod ID
        $lModelName = $pGrid[2,$i].Value                    # column 2 has the Model name

        # for individual models, we add the System ID to the folder name
        $lModelFullPath = $pPath+'\'+$lSysId+'_'+$lModelName

        #-------------------------------------------------------------------
        # confirm that the path is a proper repository, if it is, do nothing
        #-------------------------------------------------------------------
        if ( Test-Path -Path $lModelFullPath ) {
            try {
                $lRepoFilters = (Get-RepositoryInfo).Filters
            } catch {
                $lRepoConfirmed = init_repository $lModelFullPath #$True
            }
        } else {
            CMTraceLog -Message  '... Initializing Folder:'$lModelFullPath -Type $TypeNorm
            init_repository $lModelFullPath $True
        } # else if ( Test-Path -Path $lModelFullPath )

        #-------------------------------------------------------------
        # Now we have a repository, let's bring the reference file
        #-------------------------------------------------------------
        Set-Location -Path $lModelFullPath
        $lRes = (Add-RepositoryFilter -platform $lSysId -os $OS -osver $Script:v_OSVER -category BIOS -characteristic ssm 6>&1)
        Sync_and_Cleanup_Repository $pGrid $lModelFullPath $False      # $False = sync an individual repository

        #$lRes = (Remove-RepositoryFilter -platform $lSysId -os win10 -osver $Script:v_OSVER -category BIOS -characteristic ssm 6>&1)
        Remove-Item *.* -include *.exe,*.html,*.cva -Force      # delete the downloaded files 
        #$pGrid[4,$i].Value = $True

    } # for ( $i = 0; $i -lt $pGrid.RowCount; $i++ ) {

    Set-Location -Path $lCurrentSetLoc
    CMTraceLog -Message  '... [Empty_Rooted_Repos] - Done' -Type $TypeNorm
    ...
    for ( $iRow = 0; $iRow -lt $pGrid.RowCount; $iRow++ ) {
        ...
        if ( Test-Path $lTempRepoFolder ) {
            ...
            foreach ( $iEntry in $lProdFilters ) {
                if ( $DebugMode ) { CMTraceLog -Message "... Platform $($lProdFilters.platform) ... $($lProdFilters.operatingSystem) $($lProdFilters.category) $($lProdFilters.characteristic) - @$($lTempRepoFolder)" -Type $TypeWarn }
...
Function Check_PlatformsOSVersion  {
    ...
    CMTraceLog -Message "Checking support for $($OS)/$($pOSVersion) for selected platforms" -Type $TypeNorm
...
Function EditAddonFlag { 
    ...
    if ( $DebugMode ) { Write-Host 'Manage AddOn Entries Form' }
...
Function AddPlatformForm {
    ...
    if ($lResult -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        # And create a Product folder, if required for individual repositories
        if ( -not $pCommonFlag ) {
            # when creating a repo folder for a rooted individual model, add the SyID to the folder name
            $lnewPath = $pRepositoryFolder+'\'+$lSelectedSysID+'_'+$lSelectedModel
            init_repository $lnewPath $true
        } # if ( -not $pCommonFlag )
...
Function MainForm {
    ...
    $ShowHelp={
        ...
        Switch ($this.name) {
            "OS_Selection"     {$tip = "What Windows version to work with"}
    ...
    Foreach ($MenuItem in $Script:v_OSVALID) {
    ...
    $OSVERComboBox.add_SelectedIndexChanged( {
        ...
        Check_PlatformsOSVersion $dataGridView $Script:v_OSVER
        }
    ) # $OSVERComboBox.add_SelectedIndexChanged()
    ...
    $CellOnClick = {
        ...
        if ( $column -in @(0, 3, 4, 5, 6, 7) ) {
            ...
            switch ( $column ) {
                ...
                Default {   # here to deal with clicking on a category cell or 'AddOns'
    ...
    #----------------------------------------------------------------------------------
    if ( $DebugMode ) { Write-Host 'creating a Use List button' }
    ...
    #----------------------------------------------------------------------------------
    if ( $DebugMode ) { Write-Host 'creating log file field' }
    ...
    #----------------------------------------------------------------------------------
    # Create 'Empty Repos' button
    #----------------------------------------------------------------------------------
    if ( $DebugMode ) { Write-Host 'creating Create button' }
    $buttonCreate = New-Object System.Windows.Forms.Button
    $buttonCreate.Width = 90
    $buttonCreate.Height = 35
    $buttonCreate.Text = 'Empty Repos'
    $buttonCreate.Location = New-Object System.Drawing.Point(($FormWidth-230),($FormHeight-447))
    $buttonCreate.Name = 'Create'
    $buttonCreate.enabled = $false
    $buttonCreate.add_MouseHover($ShowHelp)

    $buttonCreate.add_click( {
        if ( $Script:v_CommonRepo ) {
            Empty_Common_Repo $dataGridView $CommonPathTextField.Text
        } else {
            Empty_Rooted_Repos $dataGridView $IndividualPathTextField.Text
        }
    } ) # $buttonCreate.add_click

    #$CM_form.Controls.AddRange(@($buttonCreate))
    ...
    $buttonSync.add_click( {
        ...
        # If needed, modify INI file with newly selected Script:v_OSVER
