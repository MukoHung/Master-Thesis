...
Function Set-PSProjectFiles {
	[Cmdletbinding(HelpURI = 'https://smitpi.github.io/Private-Repository/Set-PSProjectFiles')]
	PARAM(
 ...
 try {
	...
	$issues = Compare-Object -ReferenceObject (Get-ChildItem $ModulePublicFunctions).BaseName -DifferenceObject (Get-ChildItem $Moduledocs).BaseName | Where-Object { $_.SideIndicator -like '<=' } | ForEach-Object {
		[PSCustomObject]@{
	...
	foreach ($gr in $group) {
		$issues += foreach ($item in $gr.Group) {
	...
	#-ExcludeRule PSAvoidUsingWriteHost, PSUseShouldProcessForStateChangingFunctions, PSUseSingularNouns, PSAvoidTrailingWhitespace
		
	$ScriptAnalyzerIssues = @()
	$ScriptAnalyzerIssues = foreach ($item in $Listissues) {      
	...
	$issues += Compare-Object -ReferenceObject $ModCommands -DifferenceObject $newfunction | ForEach-Object {
		[PSCustomObject]@{
			Catagory = 'Not Copied'
			File     = $_.InputObject
