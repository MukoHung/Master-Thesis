        ...
        if ($pscmdlet.ShouldProcess("Computer", "Activate license via KMS"))
        {
            ...
            foreach ($Computer in $Computers) {
                ...
                $productKey = switch -Wildcard ($osVersion) {
                    ...
                    "Microsoft Windows Server 2012 R2 Datacenter*"  { "W3GGN-FT8W3-Y4M27-J84CP-Q3VJ9" }
                ...
                Write-Verbose "Product Key: $productKey"
                 ...
                 # Get Evaluation or Full Edition, use DISM if needed (Runs only on Windows 10 and Server 2016, locally)
                $IsEvaluation = (Get-WindowsEdition -Online).Edition -match "Eval"
                if($IsEvaluation)
                {
                    Write-Verbose "Running evaluation version."
                    Set-WindowsEdition -Edition (Get-WindowsEdition -Online).Edition.Replace('Eval','')
                    Set-WindowsProductKey -ProductKey $productKey

                    Write-Warning "Existing Operating System is converted from Evaluation to Full. You need to reboot."
                    $RebootPrompt = Read-Host -Prompt "Would you like to reboot? [ Y / N]"
                    ...
                    {
                        Write-Warning "You need to reboot the computer for changes to be effective."
                    }
                }
